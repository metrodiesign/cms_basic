...
class Admin_ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->load->helper(array('cookie', 'date', 'string'));

		// initialize the database
		$this->db = $this->load->database($this->config->item('database_group_name', 'admin_ion_auth'), TRUE, TRUE);

		// initialize db tables data
		$this->tables = $this->config->item('admin_tables', 'admin_ion_auth');
		...
		$this->join = $this->config->item('admin_join', 'admin_ion_auth');
		...
		$this->messages    = array();
		$this->errors      = array();
		$delimiters_source = $this->config->item('delimiters_source', 'admin_ion_auth');
		...
		if ($this->hash_method == 'bcrypt')
		{
			...
			$params['salt_prefix'] = $this->config->item('salt_prefix', 'admin_ion_auth');
	...
	{
		$salt = random_string('alnum', $this->salt_length);
	 ...
	 * @author Supachai Pokaew
	 */
	public function salt_token($length)
	{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		else if (!$this->config->item('default_group', 'admin_ion_auth') && empty($groups))
		{
		...
		$query = $this->db->get_where($this->tables['groups'], array('name' => $this->config->item('default_group', 'admin_ion_auth')), 1)->row();
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				$token = $this->salt_token(32);

				$this->update_last_token($user->id, $token);

				$this->set_token_session($token);
	...
	{
		$recheck = (NULL !== $this->config->item('recheck_timer', 'admin_ion_auth')) ? $this->config->item('recheck_timer', 'admin_ion_auth') : 0;

		if ($recheck !== 0)
		{
			$last_login = $this->session->userdata('admin_last_check');
			if ($last_login + $recheck < time())
			{
				$query = $this->db->select('id')
								  ->where(array($this->identity_column => $this->session->userdata('admin_identity'), 'active' => '1'))
								  ->limit(1)
								  ->order_by('id', 'desc')
								  ->get($this->tables['users']);
				if ($query->num_rows() === 1)
				{
					$this->session->set_userdata('admin_last_check', time());
				}
				else
				{
					...
					{
						$this->session->unset_userdata(array($identity => '', 'admin_id' => '', 'admin_user_id' => ''));
					}
					else
					{
						$this->session->unset_userdata(array($identity, 'admin_id', 'admin_user_id'));
		...
		return (bool)$this->session->userdata('admin_identity');
	}
	...
	{
		if ($this->config->item('track_login_attempts', 'admin_ion_auth'))
		{
			$max_attempts = $this->config->item('maximum_login_attempts', 'admin_ion_auth');
			if ($max_attempts > 0)
			{
	...
	public function get_attempts_num($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'admin_ion_auth'))
		{
			...
			if ($this->config->item('track_login_ip_address', 'admin_ion_auth'))
			{
				if (!isset($ip_address))
				{
					$ip_address = $this->_prepare_ip($this->input->ip_address());
			...
			$this->db->where('time >', time() - $this->config->item('lockout_time', 'admin_ion_auth'), FALSE);
	...
	{
		if ($this->config->item('track_login_attempts', 'admin_ion_auth') && $this->config->item('track_login_ip_address', 'admin_ion_auth'))
		{
	...
	public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'admin_ion_auth'))
		{
			...
			$old_attempts_expire_period = max($old_attempts_expire_period, $this->config->item('lockout_time', 'admin_ion_auth'));
	...
	public function user($id = NULL)
	{
		...
		$id = isset($id) ? $id : $this->session->userdata('admin_user_id');
	...
	public function get_users_groups($id = FALSE)
	{
		...
		// if no id was passed use the current users id
		$id || $id = $this->session->userdata('admin_user_id');
	...
	public function add_to_group($group_ids, $user_id = FALSE)
	{
		...
		// if no id was passed use the current users id
		$user_id || $user_id = $this->session->userdata('admin_user_id');
	 ...
	 * @author Ben Edmunds
	 */
	public function update_last_token($id, $token)
	{
		$this->trigger_events('update_last_token');

		$this->trigger_events('extra_where');

		$this->db->update($this->tables['users'], array('token' => $token, 'token_time' => time()), array('id' => $id));
	...
	public function set_lang($lang = 'en')
	{
		...
		if ($this->config->item('user_expire', 'admin_ion_auth') === 0)
		{
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    'admin_identity'             => $user->{$this->identity_column},
		    'admin_email'                => $user->email,
		    'admin_user_id'              => $user->id,
		    'admin_old_last_login'       => $user->last_login,
		    'admin_last_check'           => time()
		);
	...
	{
		$this->trigger_events('pre_set_token_session');
		...
		$this->trigger_events('post_set_token_session');
	...
	public function remember_user($id)
	{
		...
		if ($this->db->affected_rows() > -1)
		{
			...
			set_cookie(array(
			    'name'   => $this->config->item('identity_cookie_name', 'admin_ion_auth'),
			    'value'  => $user->{$this->identity_column},
			    'expire' => $expire
			));

			set_cookie(array(
			    'name'   => $this->config->item('remember_cookie_name', 'admin_ion_auth'),
			    'value'  => $salt,
			    'expire' => $expire
			));
	...
	public function login_remembered_user()
	{
		...
		if (!get_cookie($this->config->item('identity_cookie_name', 'admin_ion_auth'))
			|| !get_cookie($this->config->item('remember_cookie_name', 'admin_ion_auth'))
			|| !$this->identity_check(get_cookie($this->config->item('identity_cookie_name', 'admin_ion_auth'))))
		{
		...
		$query = $this->db->select($this->identity_column . ', id, email, last_login')
						  ->where($this->identity_column, urldecode(get_cookie($this->config->item('identity_cookie_name', 'admin_ion_auth'))))
						  ->where('remember_code', get_cookie($this->config->item('remember_cookie_name', 'admin_ion_auth')))
						  ->where('active', 1)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if ($this->config->item('admin_group', 'admin_ion_auth') === $group->name && $group_name !== $group->name)
		{
	...
	public function delete_group($group_id = FALSE)
	{
		...
		if($group->name == $this->config->item('admin_group', 'admin_ion_auth'))
		{
	 ...
	 * @return bool
	 */
	public function recheck_token($email, $token)
	{
		$recheck = (NULL !== $this->config->item('recheck_timer', 'admin_ion_auth')) ? $this->config->item('recheck_timer', 'admin_ion_auth') : 0;

		if ($recheck !== 0)
		{
			$last_check = $this->session->userdata('admin_last_check');

			if ($last_check + $recheck < time())
			{
				$this->db->select('id');
